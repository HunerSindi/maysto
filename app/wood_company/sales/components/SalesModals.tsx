    ...
    const renderAddForm = () => (
        <form onSubmit={handleAdd} className="space-y-4">
            ...
            <div className="bg-gray-50 border border-gray-300 p-3 rounded-sm">
                <label className={labelClass}>Customer Selection</label>
                ...
                {!selectedCustomer ? (
                    <div className="relative">
                        ...
                        {isSearching && <div className="text-[10px] text-gray-500 mt-1">Searching...</div>}
                    </div>
                ) : (
                    <div className="flex justify-between items-center bg-green-50 border border-green-200 p-2">
                        ...
                        <button type="button" onClick={() => { setSelectedCustomer(null); setFormData({ ...formData, customer_id: 0 }); }} className="text-[10px] uppercase text-red-600 font-bold hover:underline">Change</button>
                    </div>
                )}
            </div>
            ...
            <div className={!selectedCustomer ? 'opacity-50 pointer-events-none' : ''}>
                ...
                <div className="mb-3">
                    <label className={labelClass}>Note / Items Sold</label>
                    <textarea className="w-full border border-gray-400 p-2 text-sm focus:border-green-600 outline-none rounded-sm min-h-[80px]" value={formData.note} onChange={e => handleChange('note', e.target.value)} placeholder="e.g. 5000 sheets..." />
                </div>
    ...
    const renderEditForm = () => (
        <form onSubmit={handleUpdate} className="space-y-4">
            <div className="bg-gray-100 p-2 border border-gray-300 text-xs text-gray-600 italic mb-2">
                Note: Customer cannot be changed during update. Delete and recreate if needed.
            </div>
            <div>
                <label className={labelClass}>Sale Amount ($)</label>
            ...
            <button type="submit" className={`${btnClass} bg-amber-600 hover:bg-amber-700 text-white border-amber-800`}>Update Sale</button>
        </form>
    );

    return (
        <>
            <SimpleModal isOpen={isAddOpen} onClose={() => { setIsAddOpen(false); setSelectedCustomer(null); }} title="Create New Sale">
                {renderAddForm()}
            </SimpleModal>
