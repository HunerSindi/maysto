...
interface Props {
    isOpen: boolean; onClose: () => void; onSubmit: (data: Takeout) => Promise<void>;
    initialData?: Takeout | null; isEditing: boolean;
}
            ...
            <div className="bg-white border-2 border-gray-600 w-full max-w-sm flex flex-col shadow-lg">
                <div className="bg-gray-200 border-b border-gray-400 p-2 flex justify-between items-center">
                    <h2 className="font-bold text-sm uppercase text-gray-800">
                        {isEditing ? t.takeout.form.edit_title : t.takeout.form.create_title}
                    </h2>
                ...
                <form onSubmit={handleSubmit} className="p-4 bg-gray-50 flex flex-col gap-4">
                    <div>
                        <label className="block text-xs font-bold uppercase text-gray-600 mb-1">{t.takeout.form.amount}</label>
                    ...
                    <div>
                        <label className="block text-xs font-bold uppercase text-gray-600 mb-1">{t.takeout.form.note}</label>
                    ...
                    <div className="flex gap-2 border-t border-gray-300 pt-3">
                        <button type="button" onClick={onClose} className="flex-1 bg-white border border-gray-400 py-2 text-xs font-bold uppercase hover:bg-gray-200 rounded-sm">{t.takeout.form.cancel}</button>
                        <button type="submit" disabled={loading} className="flex-1 bg-blue-700 border border-blue-800 py-2 text-xs font-bold uppercase text-white hover:bg-blue-800 disabled:opacity-50 rounded-sm">{t.takeout.form.save}</button>
                    </div>
                </form>
            </div>
        </div>
