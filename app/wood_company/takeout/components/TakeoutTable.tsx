    ...
    return (
        <div className="flex flex-col h-full bg-white border border-gray-400">
            <div className="flex-1 overflow-auto relative">
                {loading && (
                    <div className="absolute inset-0 bg-white/60 z-10 flex items-center justify-center font-bold">
                        {t.takeout.table.loading}
                    </div>
                )}
                <table className="w-full text-left border-collapse">
                    <thead className="bg-gray-100 border-b border-gray-400 sticky top-0 z-0 shadow-sm">
                        <tr>
                            <th className="p-2 border-r border-gray-300 text-xs font-bold uppercase w-12 text-center">{t.takeout.table.id}</th>
                            <th className={`p-2 border-r border-gray-300 text-xs font-bold uppercase w-32 ${isRtl ? 'text-right' : 'text-left'}`}>{t.takeout.table.date}</th>
                            <th className={`p-2 border-r border-gray-300 text-xs font-bold uppercase ${isRtl ? 'text-right' : 'text-left'}`}>{t.takeout.table.note}</th>
                            <th className={`p-2 border-r border-gray-300 text-xs font-bold uppercase w-32 ${isRtl ? 'text-left' : 'text-right'}`}>{t.takeout.table.amount}</th>
                            <th className="p-2 text-center text-xs font-bold uppercase w-24">{t.takeout.table.actions}</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200 text-sm">
                        {safeData.length === 0 ? (
                            <tr><td colSpan={5} className="p-8 text-center text-gray-500 italic">{t.takeout.table.no_data}</td></tr>
                        ) : (
                            safeData.map((item) => (
            ...
            <div className="border-t border-gray-400 p-2 bg-gray-50 flex justify-between items-center">
                <span className="text-xs font-bold text-gray-600">{t.takeout.table.total}: {totalCount}</span>
                <div className="flex gap-1">
                    <button disabled={page <= 1} onClick={() => onPageChange(page - 1)} className="px-3 py-1 bg-white border border-gray-400 text-xs font-bold hover:bg-gray-100 disabled:opacity-50">{t.takeout.table.prev}</button>
                    <span className="px-3 py-1 text-xs font-bold bg-white border border-gray-400">{page}</span>
                    <button disabled={page >= totalPages} onClick={() => onPageChange(page + 1)} className="px-3 py-1 bg-white border border-gray-400 text-xs font-bold hover:bg-gray-100 disabled:opacity-50">{t.takeout.table.next}</button>
                </div>
            </div>
        </div>
    );
