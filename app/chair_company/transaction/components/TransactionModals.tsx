...
export default function TransactionModals({
    ...
    return (
        <>
            {/* ADJUST MODAL */}
            <SimpleModal isOpen={isAdjustOpen} onClose={handleCloseAdjust} title={t.chair_transaction.modals.adjust_title}>
                <form onSubmit={handleAdjust} className="space-y-4">
                    <div className="flex gap-2">
                        <button type="button" onClick={() => setType('withdraw')} className={`flex-1 h-10 border text-xs font-bold uppercase rounded-sm flex items-center justify-center gap-2 ${type === 'withdraw' ? 'bg-red-700 text-white border-red-900' : 'bg-gray-50 border-gray-300 text-gray-500'}`}>
                            {t.chair_transaction.modals.withdraw}
                        </button>
                        <button type="button" onClick={() => setType('add')} className={`flex-1 h-10 border text-xs font-bold uppercase rounded-sm flex items-center justify-center gap-2 ${type === 'add' ? 'bg-green-700 text-white border-green-900' : 'bg-gray-50 border-gray-300 text-gray-500'}`}>
                            {t.chair_transaction.modals.deposit}
                        </button>
                    </div>
                    <div>
                        <label className={labelClass}>{t.chair_transaction.modals.amount}</label>
                    ...
                    <div>
                        <label className={labelClass}>{t.chair_transaction.modals.reason}</label>
                        <input required type="text" className={inputClass} value={note} onChange={e => setNote(e.target.value)} placeholder={t.chair_transaction.modals.reason_ph} />
                    </div>
                    <button type="submit" className="w-full h-9 bg-red-700 hover:bg-red-800 text-white border border-red-900 text-xs font-bold uppercase rounded-sm shadow-sm mt-2">
                        {type === 'withdraw' ? t.chair_transaction.modals.confirm_withdraw : t.chair_transaction.modals.confirm_deposit}
                    </button>
                </form>
            </SimpleModal>

            {/* CONFIRM MODAL */}
            <SimpleModal isOpen={isConfirmOpen} onClose={() => setIsConfirmOpen(false)} title={t.chair_transaction.modals.close_title}>
                <div className="space-y-4">
                    <div className="bg-yellow-50 border border-yellow-200 p-3 text-xs text-yellow-800">
                        <strong className="uppercase">Warning:</strong> {t.chair_transaction.modals.warning}
                    </div>

                    <div className="space-y-2 text-sm text-gray-700">
                        <div className="flex justify-between border-b border-gray-200 pb-1">
                            <span>{t.chair_transaction.modals.pending_sales_in}</span>
                            <span className="font-mono font-bold text-green-700">+${dashboardData?.active_sales?.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between border-b border-gray-200 pb-1">
                            <span>{t.chair_transaction.modals.pending_expenses_out}</span>
                            <span className="font-mono font-bold text-red-700">-${dashboardData?.active_expenses?.toLocaleString()}</span>
                        </div>
                        <div className="flex justify-between pt-1 font-bold">
                            <span>{t.chair_transaction.modals.net_change}</span>
                            <span className={`font-mono ${(dashboardData?.net_profit || 0) >= 0 ? 'text-green-700' : 'text-red-700'}`}>
                    ...
                    >
                        {t.chair_transaction.modals.confirm_close}
                    </button>
                </div>
            </SimpleModal>
        </>
