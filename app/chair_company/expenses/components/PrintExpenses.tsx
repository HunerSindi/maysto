                ...
                <thead>
                    ...
                    {/* <tr>
                        <th colSpan={4} className="text-center font-bold uppercase text-xl pb-6">
                            {t.chair_expenses.title}
                        </th>
                    </tr> */}
                    <tr className="bg-gray-100">
                        <th className="border border-black p-2 text-center text-xs font-bold">{t.chair_expenses.table.id}</th>
                        <th className={`border border-black p-2 text-xs font-bold ${isRtl ? 'text-right' : 'text-left'}`}>{t.chair_expenses.table.date}</th>
                        <th className={`border border-black p-2 text-xs font-bold ${isRtl ? 'text-right' : 'text-left'}`}>{t.chair_expenses.table.category} / {t.chair_expenses.table.note}</th>
                        <th className={`border border-black p-2 text-xs font-bold ${isRtl ? 'text-left' : 'text-right'}`}>{t.chair_expenses.table.amount}</th>
                    </tr>
                </thead>
                <tfoot>
                    <tr className="bg-gray-50 font-bold">
                        <td colSpan={3} className={`border border-black p-2 text-xs uppercase ${isRtl ? 'text-left' : 'text-right'}`}>{t.chair_expenses.actions.total_expenses}:</td>
