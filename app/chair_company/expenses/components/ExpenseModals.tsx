...
import { Expense } from '@/types/chair';
...
export default function ExpenseModals({
    ...
    // Load from local storage (Unique key for Chair Company)
    useEffect(() => {
        const stored = localStorage.getItem('chair_expense_categories');
        if (stored) {
            setSavedCategories(JSON.parse(stored));
        } else {
            // Default categories
            setSavedCategories(['Metal', 'Fabric', 'Foam', 'Paint', 'Labor']);
        }
    }, []);
    ...
    const addCategory = (e: React.MouseEvent) => {
        ...
        localStorage.setItem('chair_expense_categories', JSON.stringify(updated));
        setNewCatInput('');
    };
    ...
    // Styles
    const inputClass = "w-full h-9 border border-gray-400 px-2 text-sm focus:border-red-600 outline-none rounded-sm transition-colors";
        ...
        <form onSubmit={onSubmit} className="space-y-3">
            ...
            <div>
                <div className="flex justify-between items-end mb-1">
                    <label className={labelClass}>{t.chair_expenses.modals.labels.category}</label>
                    <button
                        type="button"
                        onClick={() => setIsManagingCats(!isManagingCats)}
                        className={`flex items-center gap-1 text-[9px] font-bold uppercase px-2 py-0.5 rounded-sm border transition-colors ${isManagingCats ? 'bg-red-100 text-red-800 border-red-300' : 'bg-gray-100 text-gray-500 border-gray-300 hover:bg-gray-200'}`}
                    >
                        <Settings size={10} />
                ...
                {/* Tags Container */}
                <div className={`mb-2 bg-gray-50 p-2 border rounded-sm transition-colors ${isManagingCats ? 'border-red-300 ring-1 ring-red-100' : 'border-gray-200'}`}>
                                ...
                                className={`
                                    ...
                                    ${isManagingCats
                                        ...
                                        : formData.category === cat
                                            ? 'bg-red-700 text-white border-red-900 shadow-sm cursor-pointer' // Selected Style (Red)
                                            : 'bg-white text-gray-600 border-gray-300 hover:border-red-500 hover:text-red-700 cursor-pointer' // Default Style
                                    }
                                `}
                            >
                                {cat}
                                ...
                                {isManagingCats && (
                                    <button
                                        ...
                                        className="bg-gray-100 hover:bg-red-500 hover:text-white text-red-600 rounded-full p-0.5 ml-1 transition-colors border border-gray-300"
                                        title="Delete Tag"
                                    >
                                        <X size={10} />
                                    </button>
                    ...
                    {isManagingCats && (
                        <div className="flex gap-1 mt-3 border-t border-gray-200 pt-2 animate-in fade-in slide-in-from-top-1">
                            <input
                                ...
                                className="h-7 text-xs border border-gray-300 px-2 w-full rounded-sm outline-none focus:border-red-500 bg-white"
                                autoFocus
                            />
                            <button
                                onClick={addCategory}
                ...
                <input
                    ...
                    placeholder={t.chair_expenses.modals.labels.category_ph}
                />
            </div>

            {/* --- Amount --- */}
            <div>
                <label className={`${labelClass} block mb-1`}>{t.chair_expenses.modals.labels.amount}</label>
            ...
            {/* --- Note --- */}
            <div>
                <label className={`${labelClass} block mb-1`}>{t.chair_expenses.modals.labels.note}</label>
                <textarea className="w-full border border-gray-400 p-2 text-sm focus:border-red-600 outline-none rounded-sm min-h-[80px]" value={formData.note} onChange={e => handleChange('note', e.target.value)} placeholder={t.chair_expenses.modals.labels.note_ph} />
            </div>
    ...
    return (
        <>
            <SimpleModal isOpen={isAddOpen} onClose={closeAll} title={t.chair_expenses.modals.new_title}>
                {renderForm(handleAdd, t.chair_expenses.modals.buttons.create, "bg-red-700 hover:bg-red-800 border-red-900")}
            </SimpleModal>

            <SimpleModal isOpen={isEditOpen} onClose={closeAll} title={t.chair_expenses.modals.edit_title}>
                {renderForm(handleUpdate, t.chair_expenses.modals.buttons.update, "bg-amber-600 hover:bg-amber-700 border-amber-800")}
            </SimpleModal>

            <SimpleModal isOpen={isDeleteOpen} onClose={closeAll} title={t.chair_expenses.modals.delete_title}>
                <div className="space-y-4">
                    <p className="text-sm text-gray-700">
                        {t.chair_expenses.modals.delete_text} <strong className="font-mono">${selectedExpense?.amount}</strong>? {t.chair_expenses.modals.enter_code}
                    </p>
                    ...
                    <button onClick={handleDelete} disabled={userDeleteInput !== deleteCode} className={`${btnClass} bg-red-700 text-white hover:bg-red-800 disabled:opacity-50`}>{t.chair_expenses.modals.buttons.confirm_delete}</button>
                </div>
            </SimpleModal>
        </>
    );
