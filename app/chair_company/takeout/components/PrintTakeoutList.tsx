...
import { chairHeader } from "@/app/image/chair-header";

export default function PrintTakeoutList({ data, totalSum }: { data: ChairTakeout[], totalSum: number }) {
    const { t, language } = useLanguage();
    const isRtl = language === 'ar' || language === 'ku';

    // Guard: Prevent "map of null"
    const safeData = data || [];
                    ...
                    <tr>
                        <th colSpan={4} className="pb-6">
                            <div className="flex justify-center p-0 m-0">
                                <img src={chairHeader} className="" alt="Header" />
                            </div>
                        </th>
                    </tr>
                    {/* <tr>
                        <th colSpan={4} className="text-center font-bold uppercase text-xl pb-6">
                            {t.chair_takeout.title}
                        </th>
                    </tr> */}
                    <tr className="bg-gray-100">
                        <th className="border border-black p-2 text-center text-xs font-bold w-12">{t.chair_takeout.table.id}</th>
                        <th className={`border border-black p-2 text-xs font-bold w-32 ${isRtl ? 'text-right' : 'text-left'}`}>{t.chair_takeout.table.date}</th>
                        <th className={`border border-black p-2 text-xs font-bold ${isRtl ? 'text-right' : 'text-left'}`}>{t.chair_takeout.table.note}</th>
                        <th className={`border border-black p-2 text-xs font-bold w-32 ${isRtl ? 'text-left' : 'text-right'}`}>{t.chair_takeout.table.amount}</th>
                    </tr>
                </thead>
                <tfoot>
                    <tr className="bg-gray-50 font-bold">
                        <td colSpan={3} className={`border border-black p-2 text-xs uppercase ${isRtl ? 'text-left' : 'text-right'}`}>{t.chair_takeout.filters.total_sum}:</td>
