...
import { getChairTakeouts, createChairTakeout, updateChairTakeout, deleteChairTakeout, ChairTakeout } from "@/lib/api/chair-takeout";
import { useLanguage } from "@/lib/i18n/LanguageContext";
import { Plus, Printer } from "lucide-react";

// Imports
// Assuming you have a header for Chair Company similar to WoodHeader
import ChairHeader from "@/app/chair_company/components/ChairHeader";
...
export default function ChairTakeoutPage() {
    const { t } = useLanguage();

    // State
    const [data, setData] = useState<ChairTakeout[]>([]);
    ...
    const [selectedItem, setSelectedItem] = useState<ChairTakeout | null>(null);
    ...
    const loadData = async () => {
        setLoading(true);
        const res = await getChairTakeouts(page, limit, search);
        if (res) {
            setData(res.data);
            setMeta(res.meta);
        } else {
            setData([]); // Handle error by clearing data
        }
        setLoading(false);
    };

    // Handlers
    const handleSave = async (item: ChairTakeout) => {
        if (selectedItem && selectedItem.id) {
            await updateChairTakeout(selectedItem.id, item);
        } else {
            await createChairTakeout(item);
        }
        setIsFormOpen(false);
        loadData();
    };
    ...
    const handleDelete = async (id: number) => {
        if (confirm(t.chair_takeout.dialog.delete_confirm)) {
            await deleteChairTakeout(id);
            loadData();
        }
    };
            ...
            <div className="flex-1 flex overflow-hidden p-2 gap-2 print:hidden">
                ...
                <div className="flex-1 bg-white border border-gray-400 p-2 flex flex-col gap-4 h-fit">
                    <div className="bg-gray-50 border border-gray-200 p-3 text-center">
                        ...
                        >
                            <Plus size={18} /> {t.chair_takeout.actions.add}
                        </button>
                        <button
                        ...
                        >
                            <Printer size={18} /> {t.chair_takeout.actions.print}
                        </button>
                    </div>

                    <div className="bg-red-50 border border-red-200 p-3">
                        <h3 className="text-xs font-bold uppercase text-red-700 mb-1">
                            {t.chair_takeout.filters.total_sum}
                        </h3>
