...
export default function SalesModals({
    ...
    const renderForm = (onSubmit: any, btnText: string, btnColor: string) => (
        <form onSubmit={onSubmit} className="space-y-4">
            <div>
                <label className={labelClass}>{t.chair_sales.modals.amount_label}</label>
            ...
            <div>
                <label className={labelClass}>{t.chair_sales.modals.note_label}</label>
                <textarea className="w-full border border-gray-400 p-2 text-sm focus:border-red-600 outline-none rounded-sm min-h-[80px]" value={formData.note} onChange={e => handleChange('note', e.target.value)} placeholder={t.chair_sales.modals.note_ph} />
            </div>
    ...
    return (
        <>
            <SimpleModal isOpen={isAddOpen} onClose={() => setIsAddOpen(false)} title={t.chair_sales.modals.new_title}>
                {renderForm(handleAdd, t.chair_sales.modals.btn_create, "bg-green-700 hover:bg-green-800 border-green-900")}
            </SimpleModal>

            <SimpleModal isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} title={t.chair_sales.modals.edit_title}>
                {renderForm(handleUpdate, t.chair_sales.modals.btn_update, "bg-amber-600 hover:bg-amber-700 border-amber-800")}
            </SimpleModal>

            <SimpleModal isOpen={isDeleteOpen} onClose={() => setIsDeleteOpen(false)} title={t.chair_sales.modals.delete_title}>
                <div className="space-y-4">
                    <p className="text-sm text-gray-700">{t.chair_sales.modals.delete_text} <strong className="font-mono text-green-700">${selectedSale?.amount}</strong>? {t.chair_sales.modals.enter_code}</p>
                    ...
                    <button onClick={handleDelete} disabled={userDeleteInput !== deleteCode} className={`${btnClass} bg-red-700 text-white hover:bg-red-800 disabled:opacity-50`}>{t.chair_sales.modals.btn_delete}</button>
                </div>
            </SimpleModal>
        </>
    );
