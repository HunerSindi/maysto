        ...
        >
            <table className="w-full border-collapse font-sans text-black table-fixed">
                ...
                <thead>
                    ...
                    {/* <tr><th colSpan={4} className="text-center text-xl font-bold uppercase pb-6">{t.chair_sales.title}</th></tr> */}
                    <tr className="bg-gray-100">
                        <th className="border border-black p-2 text-center text-xs font-bold">{t.chair_sales.table.id}</th>
                        <th className={`border border-black p-2 text-xs font-bold ${isRtl ? 'text-right' : 'text-left'}`}>{t.chair_sales.table.date}</th>
                        <th className={`border border-black p-2 text-xs font-bold ${isRtl ? 'text-right' : 'text-left'}`}>{t.chair_sales.table.note}</th>
                        <th className={`border border-black p-2 text-xs font-bold ${isRtl ? 'text-left' : 'text-right'}`}>{t.chair_sales.table.amount}</th>
                    </tr>
                </thead>
                <tfoot>
                    <tr className="border border-black font-bold bg-gray-50">
                        <td colSpan={3} className={`border border-black p-2 text-xs uppercase ${isRtl ? 'text-left' : 'text-right'}`}>{t.chair_sales.actions.total_revenue}:</td>
