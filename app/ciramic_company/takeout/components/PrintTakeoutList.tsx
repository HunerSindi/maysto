...
export default function PrintTakeoutList({ data, totalSum }: { data: CeramicTakeout[], totalSum: number }) {
                ...
                <thead>
                    ...
                    {/* <tr>
                        <th colSpan={4} className="text-center font-bold uppercase text-xl pb-6">
                            {t.ceramic_takeout.title}
                        </th>
                    </tr> */}
                    <tr className="bg-gray-100">
                        <th className="border border-black p-2 text-center text-xs font-bold w-12">{t.ceramic_takeout.table.id}</th>
                        <th className={`border border-black p-2 text-xs font-bold w-32 ${isRtl ? 'text-right' : 'text-left'}`}>{t.ceramic_takeout.table.date}</th>
                        <th className={`border border-black p-2 text-xs font-bold ${isRtl ? 'text-right' : 'text-left'}`}>{t.ceramic_takeout.table.note}</th>
                        <th className={`border border-black p-2 text-xs font-bold w-32 ${isRtl ? 'text-left' : 'text-right'}`}>{t.ceramic_takeout.table.amount}</th>
                    </tr>
                </thead>
                <tfoot>
                    <tr className="bg-gray-50 font-bold">
                        <td colSpan={3} className={`border border-black p-2 text-xs uppercase ${isRtl ? 'text-left' : 'text-right'}`}>{t.ceramic_takeout.filters.total_sum}:</td>
